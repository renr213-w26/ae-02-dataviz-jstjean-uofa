...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013))+
  geom_point(color = "hotpink") + 
  labs(
    x = "Movie budget (in 2013 $)", 
    y = "Gross revenue (in 2013 $)", 
    title = "Budget vs Revenue of Movies in 2013 "
  )
```
...
#| warning: false 
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, 
                    colour = binary, size = roi, alpha = 0.5)) + 
  geom_point() + 
...
I don't think it adds much overall because only the "ok" category passes so having a whole column for pass doesn't make a lot of sense.
...
#| warning: false 
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, 
                    colour = binary, size = roi, alpha = 0.3)) + 
  geom_point() + 
  facet_grid(rows = vars(clean_test), cols = vars(binary)) +
  labs(
    x = "Budget (in million 2013 $)", 
...
ggplot(bechdel, 
       aes(x = clean_test, y = roi, colour = binary)) +
         geom_boxplot() +
  labs(
    title = "Return on investement vs. Bechdel test result",
...
There doesn't seem to be a lot of difference between the return on investment between the movies that do and do not pass the Bechdel test.
Some of the non-passing movies seem to do slightly better but there is no major loss of roi when it does pass the test.
So it would make sense to just make movies that pass.
